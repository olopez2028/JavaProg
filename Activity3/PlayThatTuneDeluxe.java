...
public class PlayThatTuneDeluxe {
    ...
    public static double[] note(int pitch, double t) {
        ...
        double[] a  = tone(hz, t);     //1 tone
        double[] hi = tone(2*hz, t);   //2 tone
        double[] lo = tone(hz/2, t);   //1/2 tone        
        double[] octave = tone(hz/8, t); //1/8 tone
        double[] quart = tone(hz/4, t); //1/4 tone
                
        double[] h1  = sum(hi, lo, .5, .5); 
        
        double[] h2 = sum(quart, octave, .5, .5);
